...
public class ConstantAnalysis {
  ...
  private final Map<Local, Object> constSolution;
  private final Multimap<Local, Local> jumpSolution; // local flow-to set
  // utils
  private final Hierarchy hier = Hierarchy.v();
  private final QueryHelper queryHelper = QueryHelper.v();
  private final AndroidCallGraph cg = AndroidCallGraph.v();
    ...
    if (s instanceof DefinitionStmt) {
      ...
      if (lop instanceof Local) {
        ...
        if (lookForRef && llType instanceof RefType) {
          ...
          return hier.isSubclassOf(guiClz, llocalClz);
        } else if (!lookForRef) {
  ...
  private boolean isIntType(Value v) {
    ...
    if (type instanceof IntType) {
      ...
      return sc == Scene.v().getSootClass("java.lang.Integer");
    }
    return false;
  }
